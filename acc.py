...
if __name__ == '__main__':
    ...
    argparser.add_argument('--prompt_fp', type=str, default=r'F:\pythonProject\geval-main\prompts\summeval\acc_rank.txt')
    argparser.add_argument('--save_fp', type=str, default=r'F:\pythonProject\geval-main\results\4o\40_acc_sy.json')
    ...
    with open(args.reference, "r", encoding="utf-8") as file6:
        reference_json = json.load(file6)
    ...
    for key, value in candidates1_json.items():
        if int(key)<10000:
            ...
            reference = str(reference_json[key])
            #print(reference)
            cur_prompt = prompt.replace('{{reference}}',reference).replace('{{candidate1}}', candidate1).replace('{{candidate2}}', candidate2).replace('{{candidate3}}', candidate3).replace('{{candidate4}}', candidate4).replace('{{candidate5}}', candidate5)
            # print(cur_prompt)
